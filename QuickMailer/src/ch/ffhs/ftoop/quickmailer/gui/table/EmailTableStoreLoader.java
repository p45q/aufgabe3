package ch.ffhs.ftoop.quickmailer.gui.table;
...
import ch.ffhs.ftoop.quickmailer.mail.ReceiveMail;
import ch.ffhs.ftoop.quickmailer.storage.StorageService;



/**
 * Class EmailTableStoreLoader
 * 
...
public class EmailTableStoreLoader extends SwingWorker<ModelEmailTable, Mail> {
	...
	public EmailTableStoreLoader(MailFolder selectedFolder,
			ModelEmailTable mailTableModel, JLabel progressLabel, Boolean getNew) {
	...
	private void getNewMails() throws Exception {
		...
		if (mailList == null) {
			...
			updateProgress("Inbox has been updated successfully, "
					+ mailList.size() + " new e-mails");
		}
	}
	...
	private void getMailFromFolder() {
		...
		updateProgress("Folder <" + selectedFolder.getLabel()
